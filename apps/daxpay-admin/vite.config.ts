...
// 通行密钥(WebAuthn)要求安全上下文, 移动端真机测试需 dev server 开启 HTTPS。
// 证书放在仓库根 certs/ 下(已 gitignore), 证书文件存在即启用 HTTPS, 不存在则维持 HTTP, 不影响其他协作者。
  ...
  return {
    ...
    vite: {
      server: {
        ...
        allowedHosts: true,
        // 自签证书存在时启用 HTTPS(Passkey 真机调试用; 注意 WebAuthn 的 rpId 不能是 IP, 手机须以 localhost 或域名访问)
        https:
