...
export const DouyinMchAppCapabilityApi = {
  /** 查询通道商户的能力应用关联列表 */
  listByChannelMchNo(mchNo: string, channelMchNo: string): Promise<Result<DouyinMchAppCapability[]>> {
    return defHttp.get({
      url: '/admin/douyin/mch-app/capability/list-by-channel-mch-no',
      params: { mchNo, channelMchNo },
    });
  },
  /** 全量保存能力应用关联 */
  saveBatch(data: DouyinMchAppCapabilityBatchParam): Promise<Result<void>> {
    return defHttp.post({ url: '/admin/douyin/mch-app/capability/save-batch', data });
  },
  /** 查询抖音直连支持的支付能力候选 */
  listSupportedCapabilities(): Promise<Result<DouyinCapabilityOption[]>> {
    return defHttp.get({ url: '/admin/douyin/mch-app/capability/list-supported-capabilities' });
  },
};
