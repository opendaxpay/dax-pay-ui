...
export const PayTradeApi = {
  ...
  page(params: PayTradeQuery & { current?: number; size?: number }): Promise<Result<PageResult<PayTradeResult>>> {
    return defHttp.get({ url: '/admin/order/pay-trade/page', params });
  },
   ...
   */
  exportExcel(query: PayTradeQuery, options: { failedText: string; fileName: string }): Promise<boolean> {
    return downloadExportFile({
      client: requestClient,
      url: '/admin/order/pay-trade/export',
      params: { ...query },
  ...
  getById(id: string): Promise<Result<PayTradeResult>> {
    return defHttp.get({ url: '/admin/order/pay-trade/get-by-id', params: { id } });
  },
