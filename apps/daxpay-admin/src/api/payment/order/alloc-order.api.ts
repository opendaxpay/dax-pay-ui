...
export const AllocOrderApi = {
  ...
  page(
    ...
    return defHttp.get({ url: '/admin/alloc/page', params });
  },
  /** 根据ID查询分账订单详情 */
  getById(id: string): Promise<Result<AllocOrderResult>> {
    return defHttp.get({ url: '/admin/alloc/get-by-id', params: { id } });
  },
  /** 发起分账 */
  create(data: AllocParam): Promise<Result<AllocCreateResult>> {
    return defHttp.post({ url: '/admin/alloc/create', data });
  },
  /** 同步分账状态(主动向通道查询) */
  sync(allocNo: string): Promise<Result<void>> {
    return defHttp.post({ url: '/admin/alloc/sync', params: { allocNo } });
  },
};
