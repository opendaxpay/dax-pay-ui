<script lang="ts" setup>
  ...
  import {
    ShengIsvChannelMerchantApi,
    type ShengIsvChannelMerchantCreateParam,
  } from '#/api/payment/channel/sheng/isv.api';
  import ChannelLogo from '#/components/channel/ChannelLogo.vue';
  import { productI18nMap, productNameMap } from '#/enums/payment';
  import { useMessage } from '#/hooks/useMessage';

  defineOptions({ name: 'ShengIsvMchCreateConfig' });
  ...
  const rules = computed(() => ({
    ...
    // 子商户号(服务商模式下必填)
    subMchId: [{ required: true, message: $t('payment.channel.shengIsv.validation.subMchId') }],
  }));
  ...
  function submit(param: Record<string, any>): Promise<any> {
    ...
    return ShengIsvChannelMerchantApi.create(param as ShengIsvChannelMerchantCreateParam);
  }
    ...
    <div v-if="!createSuccess">
      <a-spin :spinning="submitLoading">
        ...
        >
          ...
          <!-- 国际化：子商户号(服务商模式下必填) -->
          <a-form-item
            :label="$t('payment.channel.shengIsv.subMchId')"
            name="subMchId"
            :tooltip="$t('payment.channel.shengIsv.subMchIdTooltip')"
          >
            <a-input v-model:value="form.subMchId" :placeholder="$t('payment.channel.shengIsv.subMchIdPlaceholder')" />
          </a-form-item>
          <!-- 国际化：盛付通分配的应用ID -->
          <a-form-item
