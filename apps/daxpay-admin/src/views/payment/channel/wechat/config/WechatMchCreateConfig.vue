...
const subMchIdLabel = computed(() =>
  isDirectProduct.value
    ? $t('payment.channel.wechatPay.mchId')
    : $t('payment.channel.wechatIsv.subMchId'),
);

const subMchIdPlaceholder = computed(() =>
  isDirectProduct.value
    ? $t('payment.channel.wechatPay.mchIdPlaceholder')
    : $t('payment.channel.wechatIsv.subMchIdPlaceholder'),
);
...
const rules = computed(() => ({
  ...
  subMchId: [
    {
      required: true,
      message: isDirectProduct.value
        ? $t('payment.channel.wechatPay.validation.mchId')
        : $t('payment.channel.wechatIsv.validation.subMchId'),
    },
  ],
}));
...
function submit(param: Record<string, any>): Promise<any> {
  return ChannelMerchantWechatApi.create({
    ...param,
    ...form.value,
  });
}
  ...
  <div v-if="visible">
    <div v-if="!createSuccess">
      ...
      >
        ...
        <!-- 国际化：微信商户号（直连）或特约商户号（服务商） -->
        <a-form-item :label="subMchIdLabel" name="subMchId">
          <a-input v-model:value="form.subMchId" :placeholder="subMchIdPlaceholder" />
        </a-form-item>
