<script lang="ts" setup>
  ...
  import WechatIsvMchAppCard from './WechatIsvMchAppCard.vue';
  import WechatIsvMchAppDetail from './WechatIsvMchAppDetail.vue';
  import WechatIsvMchAppEdit from './WechatIsvMchAppEdit.vue';

  defineOptions({ name: 'WechatIsvMchAppManage' });
  ...
  const appList = ref<WechatIsvMchApp[]>([]);
  ...
  function handleManage(record: WechatIsvMchApp) {
    detailRef.value?.show(mchNo.value, channelMchNo.value, record);
  }

  function handleEdit(record: WechatIsvMchApp) {
...
<template>
  ...
  <div v-else class="m-4">
    ...
    <WechatIsvMchAppEdit ref="editRef" @ok="loadAppList" />
    <WechatIsvMchAppDetail ref="detailRef" @deleted="loadAppList" />
  </div>
</template>
