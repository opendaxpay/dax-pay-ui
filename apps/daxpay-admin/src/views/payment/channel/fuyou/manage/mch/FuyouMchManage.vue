<script lang="ts" setup>
  ...
  const extraFields = computed(() => [
    { label: $t('payment.channel.fuyouIsv.fuyouMchNo'), value: channelConfig.value.fuyouMchNo || '-' },
    { label: $t('payment.channel.fuyouIsv.termNo'), value: channelConfig.value.termNo || '-' },
  ]);
  ...
  function handleCardClick(card: { key: string }) {
    if (card.key === 'basicInfo') {
      // 先加载富友专属配置, 再打开基本信息抽屉
      FuyouChannelMerchantApi.findByChannelMchNo(channelMchNo.value).then(({ data }) => {
