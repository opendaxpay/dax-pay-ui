...
import { FuyouChannelMerchantApi, type FuyouIsvChannelMerchantCreateParam } from '#/api/payment/channel/fuyou/channel-merchant.api';
import ChannelLogo from '#/components/channel/ChannelLogo.vue';
import { productI18nMap, productNameMap } from '#/enums/payment';
import { useMessage } from '#/hooks/useMessage';

defineOptions({ name: 'FuyouMchCreateConfig' });
...
const rules = computed(() => ({
  ...
  fuyouMchNo: [{ required: true, message: $t('payment.channel.fuyouIsv.validation.fuyouMchNo') }],
  termNo: [{ required: true, message: $t('payment.channel.fuyouIsv.validation.termNo') }],
}));
...
function submit(param: Record<string, any>): Promise<any> {
  return FuyouChannelMerchantApi.create(param as FuyouIsvChannelMerchantCreateParam);
}
  ...
  <div v-if="visible">
    <div v-if="!createSuccess">
      ...
      >
        ...
        <a-form-item :label="$t('payment.channel.fuyouIsv.fuyouMchNo')" name="fuyouMchNo">
          <a-input
            v-model:value="form.fuyouMchNo"
            :placeholder="$t('payment.channel.fuyouIsv.fuyouMchNoPlaceholder')"
          />
        </a-form-item>
        <a-form-item :label="$t('payment.channel.fuyouIsv.termNo')" name="termNo">
          <a-input
            v-model:value="form.termNo"
            :placeholder="$t('payment.channel.fuyouIsv.termNoPlaceholder')"
          />
        </a-form-item>
