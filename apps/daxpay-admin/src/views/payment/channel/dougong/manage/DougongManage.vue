<script lang="ts" setup>
  ...
  import DougongIsvConfigEdit from '#/views/payment/channel/dougong/config/DougongIsvConfigEdit.vue';
  import PlatformAppCapability from '#/views/payment/wx/platform/PlatformAppCapability.vue';

  defineOptions({ name: 'DougongManage' });

  const router = useRouter();
  const mchKeyEditRef = ref<InstanceType<typeof DougongIsvConfigEdit> | null>(null);
  const capabilityRef = ref<InstanceType<typeof PlatformAppCapability> | null>(null);

  /**
   * 功能卡片配置
   * 斗拱(汇付天下)为聚合服务商模式, 密钥全局唯一;
   * 产品级提供微信服务商应用入口与平台默认能力绑定(微信 JSAPI/小程序)
   */
  const functionCards = computed(() => [
    {
      group: $t('payment.channel.dougongIsv.groupConfig'),
      color: 'blue',
      cards: [
        {
          key: 'mchKey',
          title: $t('payment.channel.dougongIsv.cardMchKey'),
          icon: 'ant-design:key-outlined',
          description: $t('payment.channel.dougongIsv.cardMchKeyDesc'),
        },
      ],
    },
    {
  ...
  function handleCardClick(card: { key: string; route?: string }) {
    ...
    if (card.key === 'capabilityBinding') {
      ...
      capabilityRef.value?.show('dougong_pay');
      return;
    }
    if (card.route) {
      // 支持带 query 的完整 path
...
<template>
  <div class="space-y-12 py-4">
    ...
    <DougongIsvConfigEdit ref="mchKeyEditRef" />
