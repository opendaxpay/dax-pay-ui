<script lang="ts" setup>
  ...
  import AdapayIsvConfigEdit from '#/views/payment/channel/adapay/config/AdapayIsvConfigEdit.vue';

  defineOptions({ name: 'AdapayManage' });

  const mchKeyEditRef = ref<InstanceType<typeof AdapayIsvConfigEdit> | null>(null);
  // 当前环境(由分发页传入)
  const sandbox = ref(false);

  /**
   * 功能卡片配置
   * Adapay(汇付天下)为服务商性质聚合通道, 服务商密钥按环境全局唯一, 仅一个密钥配置入口;
   * 通道商户直连密钥(应用ID/APIKey/RSA密钥)在各商户详情页维护
   */
  const functionCards = computed(() => [
    {
      group: $t('payment.channel.adapayIsv.groupConfig'),
      color: 'blue',
      cards: [
        {
          key: 'mchKey',
          title: $t('payment.channel.adapayIsv.cardMchKey'),
          icon: 'ant-design:key-outlined',
          description: $t('payment.channel.adapayIsv.cardMchKeyDesc'),
        },
      ],
    },
  ]);
  ...
  /** 初始化（由分发页调用, 平台为唯一服务商, 服务商配置按环境区分） */
...
<template>
  <div class="space-y-12 py-4">
    ...
    <AdapayIsvConfigEdit ref="mchKeyEditRef" />
  </div>
</template>
