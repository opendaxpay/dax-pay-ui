<script lang="ts" setup>
  import type { StripeChannelMerchantCreateParam } from '#/api/payment/channel/stripe/channel-merchant.api';
  ...
  defineOptions({ name: 'StripeMchCreateConfig' });
  ...
  const rules = {
    ...
    accountId: [{ required: true, message: $t('payment.channel.stripe.validation.accountId') }],
  };
  ...
  function handleSubmit() {
    formRef.value
      ...
      .then(() => {
        ...
        StripeChannelMerchantApi.create({
          mchNo: mchNo.value,
          product: productCode.value,
          ...form.value,
        } as StripeChannelMerchantCreateParam)
          .then(() => {
    ...
    <div v-if="!createSuccess">
      <a-spin :spinning="submitLoading">
        ...
        >
          ...
          <!-- 国际化: Stripe 账户 ID -->
          <a-form-item :label="$t('payment.channel.stripe.accountId')" name="accountId">
            <a-input v-model:value="form.accountId" :placeholder="$t('payment.channel.stripe.accountIdPlaceholder')" />
          </a-form-item>
