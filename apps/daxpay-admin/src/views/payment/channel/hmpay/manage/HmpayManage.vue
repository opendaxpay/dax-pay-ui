<script lang="ts" setup>
  ...
  import HmpayIsvConfigEdit from '#/views/payment/channel/hmpay/config/HmpayIsvConfigEdit.vue';
  import PlatformAppCapability from '#/views/payment/wx/platform/PlatformAppCapability.vue';

  defineOptions({ name: 'HmpayManage' });

  const router = useRouter();
  const mchKeyEditRef = ref<InstanceType<typeof HmpayIsvConfigEdit> | null>(null);
  ...
  /**
   * 功能卡片配置
   * 河马付(杉德)为聚合服务商模式, 密钥全局唯一;
   * 产品级提供微信服务商应用入口与平台默认能力绑定(微信 JSAPI/小程序)
   */
  const functionCards = computed(() => [
    {
      group: $t('payment.channel.hmpayIsv.groupConfig'),
      color: 'blue',
      cards: [
        {
          key: 'mchKey',
          title: $t('payment.channel.hmpayIsv.cardMchKey'),
          icon: 'ant-design:key-outlined',
          description: $t('payment.channel.hmpayIsv.cardMchKeyDesc'),
        },
      ],
    },
    {
  ...
  function handleCardClick(card: { key: string; route?: string }) {
    ...
    if (card.key === 'capabilityBinding') {
      ...
      capabilityRef.value?.show('hm_pay');
      return;
    }
    if (card.route) {
      // 支持带 query 的完整 path
...
<template>
  <div class="space-y-12 py-4">
    ...
    <HmpayIsvConfigEdit ref="mchKeyEditRef" />
