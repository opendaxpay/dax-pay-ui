<script lang="ts" setup>
  ...
  const formRef = ref();
  const formState = ref<WxPlatformAppAuthConfig>({});
  const originalForm = ref<WxPlatformAppAuthConfig>({});

  const canEdit = computed(() => hasPermission(PermCodes.Payment.Wx.PlatformApp.MANAGE));
  ...
  function loadConfig() {
    if (!props.wxPlatformAppId) return;
    loading.value = true;
    WxPlatformAppApi.findAuthConfigByAppId(props.wxPlatformAppId)
      .then(({ data }) => {
        formState.value = {
          ...data,
          wxPlatformAppId: props.wxPlatformAppId,
        };
  ...
  async function handleSave() {
    ...
    confirm({
      ...
      onOk() {
        ...
        const submitData: WxPlatformAppAuthConfig = {
          ...formState.value,
          ...sensitiveData,
          wxPlatformAppId: props.wxPlatformAppId,
        };
        return WxPlatformAppApi.saveAuthConfig(submitData)
          .then(() => {
