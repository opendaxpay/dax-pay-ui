<script lang="ts" setup>
  ...
  import { IsvUserApi, type UserInfoParam } from '#/api/payment/isv-user.api';
  ...
  // 表单数据
  const formState = ref<UserInfoParam>({
    id: '',
    name: '',
    phone: '',
    email: '',
  ...
  async function show(id: string) {
    ...
    resetForm();
    // 加载用户信息
    const { data } = await IsvUserApi.findById(id);
    if (data) {
      formState.value = {
        id: data.id || '',
        name: data.name || '',
  ...
  async function handleOk() {
    ...
    try {
      await IsvUserApi.update(formState.value);
