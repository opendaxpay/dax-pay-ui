<script lang="ts" setup>
  ...
  import { IsvUserApi, type ResetPasswordForm } from '#/api/payment/isv-user.api';
  ...
  async function handleOk() {
    ...
    try {
      ...
      await (userIds.value.length === 1
        ? IsvUserApi.restartPassword(userIds.value[0]!, encryptedPassword)
        : IsvUserApi.restartPasswordBatch(userIds.value, encryptedPassword));
