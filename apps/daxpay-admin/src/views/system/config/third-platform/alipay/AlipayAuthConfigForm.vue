<script lang="ts" setup>
  ...
  const formRules = computed(() => ({
    ...
    appCert: isCertMode.value ? [{ required: true, message: $t('system.thirdPlatform.alipay.validate.appCert') }] : [],
...
<template>
  <a-spin :spinning="loading" class="w-full">
    <!-- 用途提示 -->
    <div class="mb-4">
      <a-alert :message="$t('system.thirdPlatform.alipay.usageTip')" type="info" show-icon />
    </div>
            ...
            <div class="config-item config-item--block">
              ...
              <a-radio-group v-model:value="formState.authType" button-style="solid" :disabled="!isEditing">
                <a-radio-button :value="AUTH_TYPE_KEY">{{
                  $t('system.thirdPlatform.alipay.authTypeKey')
                }}</a-radio-button>
                <a-radio-button :value="AUTH_TYPE_CERT">{{
                  $t('system.thirdPlatform.alipay.authTypeCert')
                }}</a-radio-button>
              </a-radio-group>
            </div>
          </a-form-item>
