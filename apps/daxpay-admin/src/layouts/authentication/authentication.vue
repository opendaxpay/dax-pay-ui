...
<template>
  <div :class="[isDark ? 'dark' : '']" class="flex min-h-full flex-1 overflow-x-hidden select-none">
    ...
    <slot name="logo">
      <!-- 头部 Logo 和应用名称 -->
      <div v-if="logoSrc || appName" class="absolute top-0 left-0 z-10 flex flex-1" @click="clickLogo">
        <div class="mt-4 ml-4 flex flex-1 items-center text-foreground sm:top-6 sm:left-6 lg:text-foreground">
    ...
    <div v-if="!authPanelCenter" class="relative hidden w-0 flex-1 lg:block">
      <div class="absolute inset-0 size-full bg-background-deep dark:bg-[#070709]">
        ...
        >
          ...
          <div class="mt-6 text-2xl font-sans text-foreground lg:text-3xl">
            {{ pageTitle }}
          </div>
          <div class="mt-3 text-lg dark:text-muted-foreground">
            {{ pageDescription }}
          </div>
        </div>
      </div>
