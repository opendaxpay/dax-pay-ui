...
export const RefundOrderApi = {
   ...
   */
  exportExcel(query: RefundOrderQuery, options: { failedText: string; fileName: string }): Promise<boolean> {
    return downloadExportFile({
      client: requestClient,
      url: '/mch/order/refund/export',
      params: { ...query },
  ...
  manualClose(id: string): Promise<Result<RefundOrderResult>> {
    return defHttp.post({ url: '/mch/order/refund/manual-close', params: { id } });
  },
};
