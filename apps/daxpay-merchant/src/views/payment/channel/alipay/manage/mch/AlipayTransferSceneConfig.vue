<script lang="ts" setup>
  ...
  import { ALIPAY_TRANSFER_SCENES, AlipayTransferSceneApi } from '#/api/payment/alipay/alipay-transfer-scene.api';
  ...
  async function loadData() {
    loading.value = true;
    try {
      const { data } = await AlipayTransferSceneApi.list(channelMchNo.value);
      dataList.value = data ?? [];
    } finally {
      loading.value = false;
    }
  ...
  function handleSetDefault(row: AlipayTransferSceneConfig) {
    ...
    confirm({
      ...
      onOk: async () => {
        await AlipayTransferSceneApi.setDefault(id);
        message.success($t('common.saveSuccess'));
        await loadData();
      },
    });
