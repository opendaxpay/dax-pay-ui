<script lang="ts" setup>
  ...
  import {
    AlipayDirectAppApi,
    type AlipayDirectAppKeyConfigParam,
  ...
  const formRef = ref();
  const formState = ref<AlipayDirectAppKeyConfigResult>({
    authType: 'public_key',
  });
  const originalForm = ref<AlipayDirectAppKeyConfigResult>({});

  const canEdit = computed(() => hasPermission(PermCodes.Channel.App.MANAGE));

  const formRules = computed(() => ({
    authType: [{ required: true, message: $t('payment.merchant.alipayDirectApp.validation.authType') }],
    privateKey: [{ required: true, message: $t('payment.merchant.alipayDirectApp.validation.privateKey') }],
    alipayPublicKey: [
      {
        required: formState.value.authType === 'public_key',
        message: $t('payment.merchant.alipayDirectApp.validation.alipayPublicKey'),
      },
    ],
    appCert: [
      {
        required: formState.value.authType === 'cert',
        message: $t('payment.merchant.alipayDirectApp.validation.appCert'),
      },
    ],
    alipayCert: [
      {
        required: formState.value.authType === 'cert',
        message: $t('payment.merchant.alipayDirectApp.validation.alipayCert'),
      },
    ],
    alipayRootCert: [
      {
        required: formState.value.authType === 'cert',
        message: $t('payment.merchant.alipayDirectApp.validation.alipayRootCert'),
      },
    ],
  }));
  ...
  async function loadConfig() {
    ...
    loading.value = true;
    try {
      const { data } = await AlipayDirectAppApi.findKeyConfig(props.alipayDirectAppId, props.sandbox ?? false);
      formState.value = {
        authType: 'public_key',
        ...data,
      };
  ...
  async function handleSave() {
    ...
    confirm({
      ...
      onOk() {
        ...
        const submitData: AlipayDirectAppKeyConfigParam = {
        ...
        return AlipayDirectAppApi.saveKeyConfig(submitData)
          .then(() => {
    ...
    <a-card variant="borderless" class="rounded-xl shadow-sm">
      <template #title>
        <span class="text-base font-semibold text-foreground">
          {{ $t('payment.merchant.alipayDirectApp.apiInvokeConfigTitle') }}
        </span>
      </template>
      <template #extra>
      ...
      <a-spin :spinning="loading">
        ...
        >
          <a-divider orientation="left">{{ $t('payment.merchant.alipayDirectApp.basicConfig') }}</a-divider>

          <!-- 国际化: 跟随通道商户环境(只读, 不可在此切换) -->
          <a-form-item :label="$t('payment.merchant.alipayDirectApp.environment')">
            <a-tag :color="sandbox ? 'orange' : 'blue'">
              {{
                sandbox
                  ? $t('payment.merchant.alipayDirectApp.sandboxEnv')
                  : $t('payment.merchant.alipayDirectApp.prodEnv')
              }}
            </a-tag>
            <span class="ml-2 text-xs text-muted-foreground">
              {{ $t('payment.merchant.alipayDirectApp.envFollowProductHint') }}
            </span>
          </a-form-item>

          <a-form-item :label="$t('payment.merchant.alipayDirectApp.aliAppId')">
          ...
          <a-form-item :label="$t('payment.merchant.alipayDirectApp.authType')" name="authType">
            <a-select
              ...
              :disabled="!isEditing"
              :placeholder="$t('payment.merchant.alipayDirectApp.authTypePlaceholder')"
              :options="authTypeDict"
            />
          </a-form-item>

          <a-divider orientation="left">{{ $t('payment.merchant.alipayDirectApp.keyConfig') }}</a-divider>

          <a-form-item
            v-if="formState.authType === 'public_key'"
            :label="$t('payment.merchant.alipayDirectApp.alipayPublicKey')"
          ...
          <template v-if="formState.authType === 'cert'">
            <a-divider orientation="left">{{ $t('payment.merchant.alipayDirectApp.certConfig') }}</a-divider>
            ...
            <a-form-item :label="$t('payment.merchant.alipayDirectApp.appCert')" name="appCert">
              ...
              >
                <a-button :disabled="!isEditing">
                  ...
                  {{ $t('payment.merchant.alipayDirectApp.uploadAppCert') }}
                </a-button>
              </a-upload>
              <a-tooltip
                v-else
            ...
            <a-form-item :label="$t('payment.merchant.alipayDirectApp.alipayCert')" name="alipayCert">
              ...
              >
                <a-button :disabled="!isEditing">
                  <template #icon><IconifyIcon icon="ant-design:upload-outlined" class="text-lg" /></template>
                  {{ $t('payment.merchant.alipayDirectApp.uploadAlipayCert') }}
                </a-button>
              </a-upload>
              <a-tooltip
                v-else
            ...
            <a-form-item :label="$t('payment.merchant.alipayDirectApp.alipayRootCert')" name="alipayRootCert">
              ...
              >
                <a-button :disabled="!isEditing">
                  <template #icon><IconifyIcon icon="ant-design:upload-outlined" class="text-lg" /></template>
                  {{ $t('payment.merchant.alipayDirectApp.uploadRootCert') }}
                </a-button>
              </a-upload>
              <a-tooltip
                v-else
          ...
          <a-divider orientation="left">{{ $t('payment.merchant.alipayDirectApp.communicateKeyConfig') }}</a-divider>

          <a-form-item :label="$t('payment.merchant.alipayDirectApp.secretKey')">
            <a-input
