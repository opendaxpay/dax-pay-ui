<script lang="ts" setup>
  ...
  import {
    AlipayDirectAppApi,
    type AlipayDirectAppAuthConfigParam,
  ...
  const formRef = ref();
  const formState = ref<AlipayDirectAppAuthConfigResult>({
    userIdType: 'openid',
  });
  ...
  const formRules = computed(() => ({
    userIdType: [{ required: true, message: $t('payment.merchant.alipayDirectApp.validation.userIdType') }],
  }));
  ...
  function loadConfig() {
    ...
    loading.value = true;
    AlipayDirectAppApi.findAuthConfig(props.alipayDirectAppId)
      .then(({ data }) => {
        formState.value = {
          userIdType: 'openid',
          ...data,
  ...
  async function handleSave() {
    ...
    confirm({
      ...
      onOk() {
        saving.value = true;
        const submitData: AlipayDirectAppAuthConfigParam = {
          alipayDirectAppId: props.alipayDirectAppId!,
          channelMchNo: props.channelMchNo!,
          userIdType: formState.value.userIdType!,
        };
        return AlipayDirectAppApi.saveAuthConfig(submitData)
          .then(() => {
    ...
    <a-card variant="borderless" class="rounded-xl shadow-sm">
      <template #title>
        ...
        <span class="text-base font-semibold text-foreground">
          {{ $t('payment.merchant.alipayDirectApp.authConfigTitle') }}
        </span>
      </template>
      <template #extra>
        ...
        >
          ...
          <a-form-item :label="$t('payment.merchant.alipayDirectApp.userIdType')" name="userIdType">
            <a-radio-group v-model:value="formState.userIdType" :disabled="!isEditing" button-style="solid">
              <a-radio value="openid">{{ $t('payment.merchant.alipayDirectApp.userIdTypeOpenid') }}</a-radio>
              <a-radio value="userid">{{ $t('payment.merchant.alipayDirectApp.userIdTypeUserid') }}</a-radio>
              <a-radio value="openid_userid">{{ $t('payment.merchant.alipayDirectApp.userIdTypeBoth') }}</a-radio>
            </a-radio-group>
          </a-form-item>
        </a-form>
      </a-spin>
