<script lang="ts" setup>
  import type { AlipayTransferConfig } from '#/api/payment/alipay/transfer-config.api';
  ...
  import { AlipayTransferConfigApi } from '#/api/payment/alipay/transfer-config.api';
  ...
  // 通道商户支付宝应用列表
  const mchApps = ref<AlipayDirectAppResult[]>([]);
  ...
  async function loadData() {
    ...
    try {
      const [config, apps] = await Promise.all([
        AlipayTransferConfigApi.findByChannelMchNo(channelMchNo.value),
  ...
  async function saveConfig() {
    ...
    try {
      ...
      // 重新加载配置回显
      const { data } = await AlipayTransferConfigApi.findByChannelMchNo(channelMchNo.value);
    ...
    <a-spin :spinning="loading || saving">
      ...
      <div v-if="displayApp" class="mb-8">
        <a-descriptions :column="1" :bordered="true" size="small">
          ...
          <a-descriptions-item :label="$t('payment.merchant.alipayDirectApp.appType')">
