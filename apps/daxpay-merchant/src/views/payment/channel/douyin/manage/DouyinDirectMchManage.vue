<script lang="ts" setup>
  ...
  /** 初始化（商户端 mchNo 由后端上下文取，仅传通道商户号与摘要） */
  ...
  function handleCardClick(card: { key: string }) {
    ...
    if (card.key === 'transferScene') {
      // 转账场景管理
      transferSceneRef.value?.show(channelMchNo.value);
    }
...
<template>
  <div>
    ...
    <DouyinTransferSceneConfig ref="transferSceneRef" :channel-mch-no="channelMchNo" />
