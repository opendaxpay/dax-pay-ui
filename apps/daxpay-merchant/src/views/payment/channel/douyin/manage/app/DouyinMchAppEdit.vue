<script lang="ts" setup>
  ...
  async function validateDouyinAppId() {
    ...
    return existsByServer(
      ...
      id,
      formEditType.value,
      (value) => DouyinMchAppApi.existsDouyinAppId(channelMchNo.value, value),
      (value, excludeId) => DouyinMchAppApi.existsDouyinAppIdNotId(channelMchNo.value, value, excludeId),
  ...
  /** 新增（商户端 mchNo 由后端上下文取） */
  function show(mchChannelNo: string) {
  ...
  /** 编辑 */
  function showEdit(mchChannelNo: string, record: DouyinMchApp) {
    channelMchNo.value = mchChannelNo;
