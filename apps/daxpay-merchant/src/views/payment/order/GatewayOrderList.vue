<script lang="ts" setup>
  ...
  async function handleExport() {
    ...
    await GatewayOrderApi.exportExcel(queryForm.value, {
      fileName: buildExportFileName($t('payment.order.gateway.title'), {
      ...
      <a-card>
        <vxe-toolbar ref="xToolbar" custom refresh :refresh-options="{ queryMethod: queryPage }">
          <template #buttons>
            <a-space>
              <a-button
                v-if="hasPermission(PermCodes.Trade.GatewayOrder.EXPORT)"
                :loading="exporting"
                @click="handleExport"
              >
