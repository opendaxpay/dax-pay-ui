<script lang="ts" setup>
  ...
  function appSelectOptions(capability: string) {
    ...
    mchApps.value
      .filter((app) => isWxAppCompatible(app.appType, capability))
      .forEach((app) => {
        options.push({
  ...
  function rowTypeTag(capability: string): { color: string; text: string } {
    const val = bindingMap.value[capability];
    if (val === ISP_DEFAULT) {
      return { color: 'default', text: $t('payment.wx.app.capabilityIspDefault') };
    }
  ...
  /** 打开弹窗（商户端无 mchNo，取当前登录商户） */
  function show(cMchNo: string, productCode: string) {
  ...
  function loadData() {
    loading.value = true;
    Promise.all([
      WxChannelAppCapabilityApi.listSupportedCapabilities(product.value),
      WxMchAppApi.listAll(),
      WxChannelAppCapabilityApi.listByChannelMchNo(channelMchNo.value),
    ])
      .then(([capRes, mchAppRes, bindRes]) => {
        ...
        (bindRes.data ?? []).forEach((item) => {
          ...
          if (item.appScope === 'merchant' && item.wxAppRefId) {
            map[item.capability] = encodeMerchantRef(item.wxAppRefId);
          }
        });
        capabilities.value.forEach((cap) => {
          if (!map[cap.code]) {
  ...
  function handleSave() {
    const items: WxChannelAppCapabilityItem[] = [];
    capabilities.value.forEach((cap) => {
