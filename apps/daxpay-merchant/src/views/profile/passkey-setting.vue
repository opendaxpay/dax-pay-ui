<script lang="ts" setup>
  ...
  // 弹窗阶段: input=输入密码 invoking=已唤起系统注册弹窗等待用户完成(内容区切换为等待提示,
  // 弹窗保持打开直到系统注册流程结束, 避免密码弹窗先关导致"流程已结束"的错觉)
  const passwordPhase = ref<'input' | 'invoking'>('input');
  ...
  async function handlePasswordConfirm() {
    ...
    try {
      ...
      // 先切换弹窗为等待提示态再唤起系统注册弹窗, 保持视觉连续
      // (系统弹窗为 OS 层 UI, 弹窗开着不影响其展示)
      passwordPhase.value = 'invoking';
      ...
      // 系统注册流程结束(成功/取消/失败)后关闭密码弹窗, 由命名弹窗或错误提示接力;
      // options 请求失败时保持输入态让用户重试
      if (passwordPhase.value === 'invoking') {
        passwordPhase.value = 'input';
...
<template>
  <a-spin :spinning="loading">
    ...
    <!-- 添加-密码确认弹窗(invoking 阶段隐藏底部按钮并切换为等待系统验证提示) -->
    <a-modal
      ...
      :mask-closable="passwordPhase !== 'invoking'"
      :keyboard="passwordPhase !== 'invoking'"
      :footer="passwordPhase === 'invoking' ? null : undefined"
      @ok="handlePasswordConfirm"
    >
      <a-form v-if="passwordPhase === 'input'" layout="vertical">
      ...
      <!-- 等待用户在系统注册弹窗中完成验证(此期间页面保持反馈, 结束后由命名弹窗或列表接力) -->
      <div v-else class="flex flex-col items-center py-8">
        <a-spin size="large" />
        <div class="mt-4 text-sm text-gray-500">{{ $t('profile.passkeyInvoking') }}</div>
      </div>
    </a-modal>
