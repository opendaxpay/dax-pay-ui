...
interface TabbarState {
  /**
   * @zh_CN 是否替换当前标签页（内部使用）
   */
  _replaceCurrentTab: boolean;
  /**
   * @zh_CN 替换标签时的插入位置索引（内部使用）
   */
...
export const useTabbarStore = defineStore('core-tabbar', {
  actions: {
    ...
    addTab(routeTab: TabDefinition): TabDefinition {
      ...
      if (this._replaceCurrentTab) {
        const newTabKey = tab.key;
        const duplicateIndex = this.tabs.findIndex((item, index) => {
          return index !== this._replaceIndex && getTabKeyFromTab(item) === newTabKey;
        });

        if (duplicateIndex === -1) {
          const replaceIndex = this._replaceIndex;
          if (replaceIndex >= 0 && replaceIndex < this.tabs.length) {
            this.tabs.splice(replaceIndex, 1);
          }
          if (replaceIndex >= 0 && replaceIndex <= this.tabs.length) {
            this.tabs.splice(replaceIndex, 0, tab);
          } else {
            this.tabs.push(tab);
          }
          if (isVisitHistory()) {
          ...
          const currentTab = toRaw(this.tabs)[duplicateIndex];
          const mergedTab = {
            ...currentTab,
            ...tab,
          ...
          if (currentTab) {
            ...
            if (Reflect.has(curMeta, 'affixTab')) mergedTab.meta.affixTab = curMeta.affixTab;
            if (Reflect.has(curMeta, 'newTabTitle')) mergedTab.meta.newTabTitle = curMeta.newTabTitle;
          }
          this.tabs.splice(duplicateIndex, 1, mergedTab);
        }

        this._replaceCurrentTab = false;
        this._replaceIndex = -1;
     ...
     * @zh_CN 设置替换当前标签页的标记
     */
    replaceCurrentTab(route: RouteLocationNormalized) {
      const currentKey = getTabKey(route);
      const currentIndex = this.tabs.findIndex(
        (item) => getTabKeyFromTab(item) === currentKey,
      );
      this._replaceCurrentTab = true;
      this._replaceIndex = currentIndex === -1 ? this.tabs.length - 1 : currentIndex;
    },
  },
  getters: {
