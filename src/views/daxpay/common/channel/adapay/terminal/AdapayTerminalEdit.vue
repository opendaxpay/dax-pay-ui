<template>
  ...
  >
    ...
    >
      ...
      <a-form-item label="费率类型" name="feeType">
        <a-radio-group v-model:value="form.feeType" button-style="solid" :disabled="showable">
          <a-radio-button value="01">标准费率线上</a-radio-button>
          <a-radio-button value="02">标准费率线下</a-radio-button>
        </a-radio-group>
      </a-form-item>
...
<script setup lang="ts">
  ...
  import { update, get, AdaPayTerminal } from './AdapayTerminal.api'
  ...
  const form = ref<AdaPayTerminal>({})

  // 校验
  const rules = computed(() => {
    return {
      status: [{ required: true, message: '请选择报备状态' }],
      feeType: [{ required: true, message: '请选择费率类型' }],
